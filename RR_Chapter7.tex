...
\begin{convention}[Self as sSet object]
\label{conv:self-sset}
When we regard a Self $\mathsf{Self}^a_\tau$ as an object of $\mathbf{sSet}$, we mean its underlying simplicial set—the $\mathsf{Hocolim}$ component from Chapter~\ref{ch:self}.
\end{convention}
...
The Nahnu is computed by taking the disjoint union of all Selves in the network, and for each co-witness event $e$, attaching a 1-simplex connecting the sites of the two witness records. This glues Selves along their points of mutual inscription.
\end{remark}
...
The Nahnu has \textbf{Presence} at $\tau$ if at least one tracked cross-journey is present (exhibits re-entry):
\[
...
The \textbf{Nahnu} at time $\tau$ is the hocolim $\mathsf{Nahnu}_\tau$ equipped with Presence and Generativity:
\[
\mathsf{Nahnu}_\tau \;:=\; (\operatorname{hocolim}(\mathbf{Diag}^{\mathcal{N}}_\tau),\; \mathsf{Presence},\; \mathsf{Generativity})
\]
\end{definition}
...
\item $\mathsf{Nahnu}_\tau = (\operatorname{hocolim}(\mathbf{Diag}^{\mathcal{N}}_\tau), \mathsf{Presence}, \mathsf{Generativity})$
